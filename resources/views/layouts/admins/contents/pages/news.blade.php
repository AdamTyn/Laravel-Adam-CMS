...
<tr>
    ...
    <td>
        <a href="{{url('admin/page-setting/news/update').'/'.$value['id']}}">{{StrShort(ToTextarea($value['detail']),0,100)}}</a>
    </td>
    <td class="am-hide-sm-only">
        {{$value['last_change']}}
    </td>
...
<script type="text/javascript">
    $('.btn-edit').click(function(){
        ...
        $('#form').attr('action','{{url('admin/page-setting/news/update')}}'+'/'+ids[1]);
    });
    $('.btn-delete').click(function(){
        var ids=this.id.split('-');
        $('#form').attr('action','{{url('admin/page-setting/news/delete')}}'+'/'+ids[1]);
    });
</script>
</div>
</div>
