                        ...
                        <div class="am-u-sm-12 am-u-md-6">
                            <div class="am-btn-toolbar">
                                <div class="am-btn-group am-btn-group-xs">
                                    <a href="{{url('admin/page-setting/employee/add')}}" class="am-btn am-btn-default am-btn-success"><span class="am-icon-plus"></span>&nbsp;新增</a>
                                </div>
                            </div>
                        </div>
                    </div>
...
<tr>
    ...
    <td>
        <a href="{{url('admin/page-setting/employee/update').'/'.$value['id']}}">{{StrShort(ToTextarea($value['detail']),0,100)}}</a>
    </td>
    <td class="am-hide-sm-only">
        {{$value['last_change']}}
    </td>
...
<script type="text/javascript">
    $('.btn-edit').click(function(){
        ...
        $('#form').attr('action','{{url('admin/page-setting/employee/update')}}'+'/'+ids[1]);
    });
    $('.btn-delete').click(function(){
        var ids=this.id.split('-');
        $('#form').attr('action','{{url('admin/page-setting/employee/delete')}}'+'/'+ids[1]);
    });
</script>
</div>
</div>
